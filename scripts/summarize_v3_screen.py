...
def _gate_delta(
    ...
    *,
    ...
    clusters = []
    corrupted_root = (
        corrupted_json.parent
        / f"{corrupted_json.stem}.predictions"
    )
    ...
    for dataset in ("meld", "emotiontalk"):
        with np.load(
            ...
            clean_index = {
                sample_id: index
                for index, sample_id in enumerate(clean["sample_ids"].tolist())
            }
            if set(clean_index) != set(corrupted["sample_ids"].tolist()):
                raise ValueError(
                    f"{condition}/{dataset} clean and corrupted sample ids differ"
                )
            order = np.asarray(
            ...
            if not np.array_equal(clean["truth"][order], corrupted["truth"]):
                raise ValueError(
                    f"{condition}/{dataset} clean and corrupted labels differ"
                )
            if not np.array_equal(
                ...
                raise ValueError(
                    f"{condition}/{dataset} clean and corrupted context ids differ"
                )
            valid = (
                clean["modality_available"][order, modality].astype(bool)
                & corrupted["modality_available"][:, modality].astype(bool)
            )
            if not valid.any():
                raise ValueError(
                    f"{condition}/{dataset} has no paired available modality rows"
                )
            ...
            clusters.extend(
                ...
                for context_id in corrupted["context_ids"][valid].tolist()
            )
    ...
    for _ in range(iterations):
        ...
        indices = np.concatenate(
            [np.flatnonzero(cluster_values == cluster) for cluster in sampled]
        )
...
def main() -> int:
    ...
    if args.command == "loss":
        decision = select_classification_loss(
            ...
            candidates={
                name: _validation(path) for name, path in candidates.items()
            },
        )
    else:
        ...
        decision = select_gate_ranking_weight(
            ...
            candidates={
                float(name): _ranking_payload(path)
