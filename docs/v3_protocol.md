...
- Balanced Softmax 与 Focal Loss 均未通过类别损失门槛，继续使用 weighted CE。
- `λ=0.05、0.10、0.20` 均能降低受损音频和视频门控，但三个扰动的平均 weighted-F1 增益均未达到预设的 0.5 个百分点。
- `ranking-decision.json` 选择 `λ=0`，没有候选通过。
- 未运行 V3 正式三种子训练或官方测试，最终部署固定为 V2。

以下内容保留为可复现协议与失败审计记录，不再作为待执行计划。
