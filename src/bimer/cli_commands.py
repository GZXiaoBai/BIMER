...
from .runtime import RuntimeSession, build_legacy_runtime, build_runtime_session
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "analyze":
            result = session.analyze(Path(args.video), args.language)
        ...
        if args.command == "serve":
            create_app(session).launch(share=args.share)
            return 0
        return 2
    finally:
        session.close()
